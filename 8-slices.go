 ...
 * interface ot sequences than arrays
 */
package main
...
func main() {
	...
	// (not the number of elements). An uninitialized slice equals to nil
	// and has length 0
	var s []string
	fmt.Println("uninit:", s, s == nil, len(s) == 0)

	// To create a slice with non-zero length, use the builtin make.
	// Here we make a slice of string of length 3 (initially zero-valued).
	...
	// to pass a capacity explicitly as an additional paramenter to make
	s = make([]string, 3)
	...
	// that we need to accept a return value form append as we may
	...
	// slice[low:high]. For exmple, this gets a slice of the
	// elements s[2], s[3], s[4].
	l := s[2:5]
	fmt.Println("sl1:", l)
